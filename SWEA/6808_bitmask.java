...
public class swea_6808_규영이와인영이의카드게임_BitMask {
	static boolean[] card;// 1-18. 전체 카드정보 저장할 배열
	// pick boolean 배열이 필요 없어짐
	...
	public static void main(String[] args) throws Exception {
		...
		for (int tc = 1; tc <= T; tc++) {
			...
			perm(0, 0, 0, 0);// 라운드번호(횟수), flag, 규영이점수, 인영이 점수
	...
	public static void perm(int round, int flag, int sum1, int sum2) {
		...
		// 한 라운드 당 9개의 숫자를 가지고 비교할건데, 이전 라운드에서 선택 안된 숫자를 선택
		for (int i = 0; i < N; i++) {// 인영이 카드를 바꿔볼것
			if((flag & 1 << i) == 0) { //i번쨰 비트가 사용 안된것
				 if(num1[round] > num2[i]) {//규영이의 카드 > 인영이의 카드 //라운드는 증가하고 규영이 총합이 증가하고 인영이
...
/*
 * if(!pick[i]) {//i번째 카드가 사용 x/ 선택이 안된, 사용 안한 카드라면 pick[i] = true;//선택하고
 * if(num1[round] > num2[i]) {//규영이의 카드 > 인영이의 카드 //라운드는 증가하고 규영이 총합이 증가하고 인영이
 * 총합은 그대로인 상태로 다시 perm(round+1, sum1 + num1[round] + num2[i], sum2);
 * 
 * }else { perm(round+1, sum1, sum2 + num1[round] + num2[i]); } pick[i] =
 * false;// 다른 조합의 경우에 이 카드가 다시 뽑힐 수 있도록 해지 }
 */
